...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE centros SET nombre_cen = ?, empresa_cen = ?, tipo_cen = ?, direccion_cen = ? WHERE id_centro = ?";
    $stmt = $pdo->prepare($sql);
    
    // Ejecutar la consulta y verificar si se actualizó correctamente
    if ($stmt->execute([$nombre_cen, $empresa_cen, $tipo_cen, $direccion_cen, $id_centro])) {
